    ...
    <>
      ...
      <nav
        className={`fixed w-full px-6 py-4 flex justify-between items-center z-50 transition-colors duration-300 ${
          scrolled ? "bg-[#2B00AA]/90 shadow-lg" : "bg-transparent"
        }`}
      >
        <h1 className="text-2xl font-bold text-[#CFF25C]">franco.dev</h1>

        <button onClick={() => setIsOpen(true)} className="space-y-2">
          <span className="block w-6 h-0.5 bg-[#CFF25C]"></span>
          <span className="block w-6 h-0.5 bg-[#CFF25C]"></span>
        </button>
      </nav>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-[#6C3FCF] text-white flex flex-col items-center justify-center text-5xl font-extrabold z-50"
          >
            <button
