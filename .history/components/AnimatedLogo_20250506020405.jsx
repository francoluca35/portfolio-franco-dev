...
export default function AnimatedLogo() {
  ...
  const [phase, setPhase] = useState("expand"); // expand | pause | contract
      ...
      interval = setInterval(() => {
        setLeft((prev) => {
          if (prev.length < leftFull.length - 1) {
            return leftFull.slice(1, prev.length + 2); // "ran"
          }
          return prev;
        });
        ...
        setRight((prev) => {
          if (prev.length < rightFull.length - 1) {
            return rightFull.slice(1, prev.length + 2); // "omputer"
          }
          return prev;
        });
        ...
        setStep((prev) => {
          ...
          if (next >= Math.max(leftFull.length - 1, rightFull.length - 1)) {
            ...
            setTimeout(() => setPhase("pause"), 1200); // espera antes de contraer
          }
          return next;
        });
      }, 100);
  ...
  return (
    <h1 className="text-2xl font-bold text-[#0fc555] tracking-wider font-mono flex items-center gap-1">
